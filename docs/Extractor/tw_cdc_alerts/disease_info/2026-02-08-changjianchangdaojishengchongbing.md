---
nav_exclude: true
title: '常見腸道寄生蟲病'
source_url: "https://www.cdc.gov.tw/Disease/SubIndex/uzjQodThOrXY_CSe5gP3Mg"
source_layer: tw_cdc_alerts
category: disease_info
date: 2026-02-08
confidence: 高
language: zh
feed_source: "disease_zh"
diseases: ["腸道寄生蟲病", "阿米巴痢疾", "梨形鞭毛蟲", "隱胞子蟲", "蛔蟲", "鉤蟲", "鞭蟲", "蟯蟲", "中華肝吸蟲", "絛蟲"]
regions: ["熱帶地區", "亞熱帶地區", "開發中國家"]
notes: "反映當地衛生條件與生活品質，包含原蟲和蠕蟲兩類病原體"
seo:
  json_ld:
    - type: WebPage
      id: 'https://epialert.weiqi.kids/Extractor/tw_cdc_alerts/disease_info/2026-02-08-changjianchangdaojishengchongbing#webpage'
      speakable:
        cssSelector:
          - .article-summary
          - .speakable-content
          - .key-takeaway
          - .key-answer
          - .expert-quote
          - .actionable-steps li
          - .faq-answer-content
    - type: Article
      id: 'https://epialert.weiqi.kids/Extractor/tw_cdc_alerts/disease_info/2026-02-08-changjianchangdaojishengchongbing#article'
      author_id: 'https://epialert.weiqi.kids/about#person'
      publisher_id: 'https://epialert.weiqi.kids#organization'
    - type: Person
      id: 'https://epialert.weiqi.kids/about#person'
      name: 'EpiAlert AI 編輯'
    - type: Organization
      id: 'https://epialert.weiqi.kids#organization'
      name: 'EpiAlert 疫情快訊'
    - type: BreadcrumbList
      items:
        - position: 1
          name: '首頁'
          url: 'https://epialert.weiqi.kids'
        - position: 2
          name: 'tw_cdc_alerts'
          url: 'https://epialert.weiqi.kids/Extractor/tw_cdc_alerts'
        - position: 3
          name: '疾病資訊'
          url: 'https://epialert.weiqi.kids/Extractor/tw_cdc_alerts/disease_info'
        - position: 4
          name: '常見腸道寄生蟲病'
          url: 'https://epialert.weiqi.kids/Extractor/tw_cdc_alerts/disease_info/2026-02-08-changjianchangdaojishengchongbing'
  ymyl:
    lastReviewed: '2026-02-08'
    reviewedBy: 'EpiAlert AI 編輯'
    medicalDisclaimer: '本站內容由 AI 自動彙整自 WHO、CDC 等官方來源,僅供參考,不構成醫療建議。如有健康疑慮,請諮詢專業醫療人員。'
---

## 摘要
腸道寄生蟲病在熱帶及亞熱帶地區普遍流行，是開發中或未開發國家常見的疾病。腸道寄生蟲的病原體包括原蟲和蠕蟲兩類，常見原蟲感染如阿米巴痢疾、梨形鞭毛蟲及隱胞子蟲等；蠕蟲感染如蛔蟲、鉤蟲、鞭蟲、蟯蟲、中華肝吸蟲及絛蟲等。疾病盛行率可間接反應當地的衛生條件與生活品質。

## 詳細內容

### 疾病概況
腸道寄生蟲病在熱帶及亞熱帶地區普遍流行，是開發中或未開發國家常見的疾病。從各個地區或國家之腸道傳染病之盛行率，可以間接的反應出當地的衛生條件與生活品質。

### 主要病原體

**原蟲感染類**：
- **阿米巴痢疾（Amoebiasis）**：由痢疾阿米巴引起
- **梨形鞭毛蟲（Giardiasis）**：引起腹瀉和吸收不良
- **隱胞子蟲（Cryptosporidiosis）**：水源性寄生蟲

**蠕蟲感染類**：
- **蛔蟲（Ascariasis）**：常見的腸道線蟲
- **鉤蟲（Hookworm）**：可導致貧血
- **鞭蟲（Trichuriasis）**：寄生於大腸
- **蟯蟲（Enterobiasis）**：常見於兒童
- **中華肝吸蟲（Clonorchiasis）**：寄生於肝膽系統
- **絛蟲（Taeniasis）**：包括豬肉絛蟲、牛肉絛蟲等

### 傳播途徑
主要透過：
- 糞口途徑（食用受污染的食物或水）
- 接觸受污染的土壤
- 食用未煮熟的食物（如生魚片、生肉）
- 衛生條件不良的環境

### 症狀
依寄生蟲種類而異，常見症狀包括：
- 腹痛、腹瀉
- 營養不良
- 貧血
- 腹部不適
- 體重減輕
- 肛門搔癢（蟯蟲）
- 肝膽系統症狀（肝吸蟲）

### 預防措施
- 保持良好個人衛生
- 勤洗手（特別是如廁後、進食前）
- 飲用煮沸或過濾的水
- 充分煮熟食物
- 避免食用生食
- 改善環境衛生
- 定期驅蟲（高風險地區）

### 診斷與治療
- 糞便檢查
- 血液檢查
- 影像學檢查
- 使用適當的驅蟲藥物

### 相關資源
該網站提供三大主要資訊區塊：

1. **疾病介紹**：詳細的醫學說明
2. **治療照護**：包括病人治療照護及寄生蟲藥物專案進口領用標準流程
3. **蟯蟲症Q&A**：針對特定寄生蟲的常見問題解答

民眾如有相關健康疑問，可撥打防疫專線 1922 或 0800-001922 諮詢。

## 相關疾病
- 阿米巴痢疾
- 梨形鞭毛蟲症
- 隱胞子蟲症
- 蛔蟲症
- 鉤蟲症
- 鞭蟲症
- 蟯蟲症
- 中華肝吸蟲感染症
- 絛蟲症

## 影響地區
- 熱帶地區
- 亞熱帶地區
- 開發中國家
- 未開發國家
- 衛生條件較差的地區

---
*萃取時間: 2026-02-08T18:30:00+08:00*
*資料來源: 衛生福利部疾病管制署*
