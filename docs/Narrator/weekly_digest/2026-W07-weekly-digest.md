---
nav_exclude: true
title: '傳染病威脅週報 2026-W07'
layout: default
parent: 週報
seo:
  meta:
    title: '傳染病威脅週報 2026-W07 - 台灣首例麻疹境外移入、百日咳確診 | EpiAlert'
    description: '本週重點：台灣今年首例麻疹境外移入病例（越南）、首例百日咳確診、歐洲麻疹社區傳播持續、禽流感 H5N1 冬季高峰期。來自 WHO、CDC、ECDC 等官方來源的完整分析。'
  json_ld:
    '@context': 'https://schema.org'
    '@graph':
      - '@type': 'WebPage'
        '@id': 'https://epialert.weiqi.kids/Narrator/weekly_digest/2026-W07-weekly-digest#webpage'
        url: 'https://epialert.weiqi.kids/Narrator/weekly_digest/2026-W07-weekly-digest'
        name: '傳染病威脅週報 2026-W07'
        description: '台灣首例麻疹境外移入、百日咳確診、歐洲麻疹社區傳播、禽流感 H5N1 監測'
        inLanguage: 'zh-TW'
        isPartOf:
          '@id': 'https://epialert.weiqi.kids#website'
        primaryImageOfPage:
          '@type': 'ImageObject'
          url: 'https://epialert.weiqi.kids/assets/images/logo.png'
        datePublished: '2026-02-15T12:00:00+08:00'
        dateModified: '2026-02-16T05:00:00+08:00'
        speakable:
          '@type': 'SpeakableSpecification'
          cssSelector:
            - '.article-summary'
            - '.speakable-content'
            - '.key-takeaway'
            - '.key-answer'
            - '.expert-quote'
            - '.actionable-steps li'
            - '.faq-answer-content'
      - '@type': 'Article'
        '@id': 'https://epialert.weiqi.kids/Narrator/weekly_digest/2026-W07-weekly-digest#article'
        mainEntityOfPage:
          '@id': 'https://epialert.weiqi.kids/Narrator/weekly_digest/2026-W07-weekly-digest#webpage'
          significantLink:
            - 'https://epialert.weiqi.kids/Extractor/tw_cdc_alerts/'
            - 'https://epialert.weiqi.kids/Extractor/ecdc_cdtr/'
        headline: '傳染病威脅週報 2026-W07'
        alternativeHeadline: 'Infectious Disease Threats Weekly Report 2026-W07'
        description: '本週重點：台灣今年首例麻疹境外移入病例（越南）、首例百日咳確診、歐洲麻疹社區傳播持續、禽流感 H5N1 冬季高峰期'
        image:
          '@type': 'ImageObject'
          url: 'https://epialert.weiqi.kids/assets/images/logo.png'
          width: 1200
          height: 630
        author:
          '@id': 'https://epialert.weiqi.kids/about#person'
        publisher:
          '@id': 'https://epialert.weiqi.kids#organization'
        datePublished: '2026-02-15T12:00:00+08:00'
        dateModified: '2026-02-16T05:00:00+08:00'
        articleSection: '週報'
        keywords: '麻疹, 百日咳, 禽流感, H5N1, 猴痘, Mpox, 流感, COVID-19, 傳染病監測, 疫情週報'
        wordCount: 6500
        inLanguage: 'zh-TW'
        isAccessibleForFree: true
        isPartOf:
          '@type': 'WebSite'
          '@id': 'https://epialert.weiqi.kids#website'
          name: 'EpiAlert 疫情快訊'
          potentialAction:
            '@type': 'SearchAction'
            target: 'https://epialert.weiqi.kids/search?q={search_term}'
            query-input: 'required name=search_term'
      - '@type': 'Person'
        '@id': 'https://epialert.weiqi.kids/about#person'
        name: 'EpiAlert AI 編輯'
        url: 'https://epialert.weiqi.kids/about'
        description: 'AI 驅動的疫情資訊彙整系統，自動收集並分析來自 WHO、CDC 等官方來源的傳染病情報'
        worksFor:
          '@id': 'https://epialert.weiqi.kids#organization'
        knowsAbout:
          - '傳染病監測'
          - '公共衛生'
          - '疫情分析'
          - '流行病學'
        hasCredential:
          - '@type': 'EducationalOccupationalCredential'
            name: 'AI 輔助醫療資訊系統'
            credentialCategory: 'Automated Health Intelligence System'
        sameAs:
          - 'https://github.com/anthropics/claude-code'
      - '@type': 'Organization'
        '@id': 'https://epialert.weiqi.kids#organization'
        name: 'EpiAlert 疫情快訊'
        url: 'https://epialert.weiqi.kids'
        logo:
          '@type': 'ImageObject'
          url: 'https://epialert.weiqi.kids/assets/images/logo.png'
          width: 600
          height: 60
        description: '全球傳染病情報自動收集與分析系統'
        sameAs:
          - 'https://github.com/anthropics/claude-code'
        contactPoint:
          '@type': 'ContactPoint'
          contactType: 'technical support'
          url: 'https://github.com/anthropics/claude-code/issues'
      - '@type': 'BreadcrumbList'
        itemListElement:
          - '@type': 'ListItem'
            position: 1
            name: '首頁'
            item: 'https://epialert.weiqi.kids'
          - '@type': 'ListItem'
            position: 2
            name: '週報'
            item: 'https://epialert.weiqi.kids/Narrator/weekly_digest/'
          - '@type': 'ListItem'
            position: 3
            name: '傳染病威脅週報 2026-W07'
            item: 'https://epialert.weiqi.kids/Narrator/weekly_digest/2026-W07-weekly-digest'
      - '@type': 'ItemList'
        '@id': 'https://epialert.weiqi.kids/Narrator/weekly_digest/2026-W07-weekly-digest#highlights'
        name: '本週傳染病威脅重點'
        description: '2026 年第 7 週全球傳染病威脅摘要'
        numberOfItems: 5
        itemListElement:
          - '@type': 'ListItem'
            position: 1
            name: '台灣今年首例麻疹境外移入病例'
            description: '北部 8 個月大男嬰於越南感染後返台確診，為 2026 年首例麻疹病例，目前已掌握 400 名接觸者進行健康監測'
          - '@type': 'ListItem'
            position: 2
            name: '台灣今年首例百日咳確定病例'
            description: '北部 5 個月大男嬰確診百日咳，全球百日咳疫情嚴峻，日本去年累計病例逾 8 萬例創歷年最高'
          - '@type': 'ListItem'
            position: 3
            name: '歐洲麻疹疫情持續傳播'
            description: 'ECDC 發布 2025 年監測分析，歐盟/歐洲經濟區共報告 7,655 例麻疹病例及 8 例死亡，80% 病例未接種疫苗'
          - '@type': 'ListItem'
            position: 4
            name: '英國更新 Mpox 流行病學概況'
            description: 'UKHSA 更新猴痘 clade Ib 及 clade IIb 疫情監測報告，持續追蹤英國病例趨勢'
          - '@type': 'ListItem'
            position: 5
            name: '禽流感 H5N1 冬季高峰期'
            description: '2025 年全球共 30 例人類 H5N1 感染（12 例死亡），台灣今年已發生 7 起禽場禽流感事件'
      - '@type': 'Table'
        '@id': 'https://epialert.weiqi.kids/Narrator/weekly_digest/2026-W07-weekly-digest#comparison'
        about: '與上週疫情比較'
        description: '新增威脅、升級警示、持續關注、已解除項目的完整比較'
  ymyl:
    lastReviewed: '2026-02-15'
    reviewedBy: 'EpiAlert AI 編輯'
    medicalDisclaimer: '本報告由自動化系統生成，僅供參考用途。報告內容基於公開資訊來源，不構成醫療建議、官方政策或專業診斷。使用者應自行驗證資訊並諮詢專業人士。'
---

# 傳染病威脅週報 2026-W07

> **免責聲明**
>
> 本報告由自動化系統生成，僅供參考用途。報告內容基於公開資訊來源，
> 不構成醫療建議、官方政策或專業診斷。使用者應自行驗證資訊並諮詢
> 專業人士。

---

## 本週重點
{: .speakable-content}

<div class="key-takeaway" markdown="1">

- **台灣今年首例麻疹境外移入病例**：北部 8 個月大男嬰於越南感染後返台確診，為 2026 年首例麻疹病例，目前已掌握 400 名接觸者進行健康監測。全球麻疹疫情嚴峻，疾管署已針對越南等 7 國發布第二級旅遊警示。(tw_cdc_alerts)

- **台灣今年首例百日咳確定病例**：北部 5 個月大男嬰確診百日咳，全球百日咳疫情嚴峻，日本去年累計病例逾 8 萬例創歷年最高，多地發現抗藥性菌株。疾管署呼籲孕婦及嬰幼兒照顧者接種 Tdap 疫苗。(tw_cdc_alerts)

- **歐洲麻疹疫情持續傳播**：ECDC 發布 2025 年監測分析，歐盟/歐洲經濟區共報告 7,655 例麻疹病例及 8 例死亡（法國 4 例、羅馬尼亞 3 例、荷蘭 1 例），80% 病例未接種疫苗，社區傳播成為主要傳播模式。(ecdc_cdtr)

- **英國更新 Mpox 流行病學概況**：UKHSA 更新猴痘 clade Ib 及 clade IIb 疫情監測報告，持續追蹤英國病例趨勢。(uk_ukhsa_updates)

- **禽流感 H5N1 冬季高峰期**：2025 年全球共 30 例人類 H5N1 感染（12 例死亡），台灣今年已發生 7 起禽場禽流感事件，疾管署呼籲落實「5要6不」原則。(tw_cdc_alerts)

</div>

---

## 與上週比較

### 新增威脅

- **台灣今年首例麻疹境外移入病例**：北部 8 個月大男嬰於越南感染後返台確診，為 2026 年首例麻疹病例。2025 年台灣累計 53 例麻疹（34 例境外移入，其中越南占 30 例）。目前已掌握 400 名接觸者，監測至 2 月 28 日。

- **台灣今年首例百日咳確定病例**：北部 5 個月大男嬰確診百日咳，已按時程接種 2 劑疫苗仍感染。全球百日咳疫情嚴峻，日本去年累計逾 8 萬例創歷年最高，多地發現抗藥性菌株。

- **英國 RSV 疫苗接種覆蓋率報告**：UKHSA 發布 RSV 孕婦疫苗及老年人疫苗接種覆蓋率數據，評估疫苗計畫成效。

### 升級警示

- **麻疹（Measles）**：
  - **歐洲**：ECDC 分析顯示社區傳播持續，2025 年 7,655 例，80% 未接種疫苗，死亡病例集中於法國、羅馬尼亞、荷蘭
  - **台灣**：今年首例境外移入病例確診，疾管署針對越南等 7 國發布第二級旅遊警示
  - **全球**：墨西哥今年 1 月已報告 740 例，美國、加拿大、亞洲多國持續有病例

- **百日咳（Pertussis）**：從上週的英國監測報告，升級為台灣今年首例確診，全球疫情嚴峻，日本、美國、澳洲等國病例數持續攀升。

- **猴痘（Mpox）監測更新**：UKHSA 更新 clade Ib 及 clade IIb 流行病學概況報告，持續追蹤英國病例趨勢。

### 持續關注

- **流感（Influenza）A(H3N2)**：台灣流感疫情維持高峰，上週（2/3-2/9）新增 32 例重症及 3 例死亡，本季累計 529 例重症、97 例死亡，公費流感疫苗剩餘約 9.3 萬劑。

- **立百病毒病（Nipah Virus Disease）**：連續三週為重點威脅，印度西孟加拉邦疫情持續，台灣預計 3 月中將其列為第五類法定傳染病。

- **禽流感 H5N1**：冬季為好發季節，2025 年全球 30 例人類感染，台灣對中國及柬埔寨維持第二級旅遊警示。

- **洋蔥伯克氏菌感染（Burkholderia stabilis）**：英國污染濕紙巾相關感染事件持續追蹤中。

### 已解除 / 降級

- **英國 MMRV 疫苗計畫指引**：上週為新增威脅，本週計畫持續實施中，降為一般追蹤。

---

## 全球疫情概覽

### 新興威脅

#### 台灣今年首例麻疹境外移入病例

疾管署於 2 月 12 日公布 2026 年首例麻疹境外移入病例：

**病例資訊**：
- **個案**：北部 8 個月大本國籍男嬰
- **感染地點**：越南（隨家人探親）
- **發病過程**：1 月底返台，2 月上旬出現發燒、咳嗽，2 月 6 日因出疹住院，經採檢確診
- **尚未達 MMR 疫苗接種年齡**

**接觸者追蹤**：
- 同住家人 5 人
- 其他接觸者 395 人
- 共計 400 人，監測至 2 月 28 日

**台灣 2025 年疫情統計**：
- 累計 53 例麻疹病例
- 境外移入 34 例（越南 30 例、俄羅斯、加拿大、印尼、泰國各 1 例）
- 國內感染 19 例

**旅遊疫情建議**：
- **第二級警示（Alert）**：越南、印尼、印度、安哥拉、墨西哥、葉門、巴基斯坦（共 7 國）
- **第一級注意（Watch）**：31 國

來源：tw_cdc_alerts/imported_case/2026-02-12-measles-imported-case-vietnam.md

##### 歷史參考 [語意搜尋]

| 日期 | 標題 | 來源 | 相關分數 |
|------|------|------|----------|
| 2025-03-27 | Measles - United States of America | WHO | 0.68 |
| 2026-01-29 | Measles Outbreak Associated with an Infectious Traveler - Colorado | US CDC MMWR | 0.66 |
| 2019-05-02 | Increase in Measles Cases - United States, January-April 2019 | US CDC MMWR | 0.63 |

#### 台灣今年首例百日咳確定病例

疾管署於 2 月 12 日公布 2026 年首例百日咳確定病例：

**病例資訊**：
- **個案**：北部 5 個月大男嬰
- **已按時程接種 2 劑百日咳疫苗**
- **發病過程**：1 月 23 日起出現咳嗽、喘鳴音、流鼻水、鼻塞，後出現陣發性咳嗽、呼吸喘，2 月 10 日確診，目前症狀已改善

**接觸者追蹤**：
- 家庭接觸者 4 人
- 其中 2 名出現疑似症狀，已安排採檢及預防性投藥
- 監測至 3 月 4 日

**全球疫情嚴峻**：
- **日本**：2025 年全年逾 8 萬例（歷年最高），多地發現抗藥性菌株；今年截至第 4 週累計逾 800 例
- **中國**：今年 1 月逾 900 例；去年約 3.5 萬例
- **韓國**：今年 1 月 62 例；去年約 5,500 例
- **美國**：今年 1 月逾 700 例；去年約 2.8 萬例
- **澳洲**：去年逾 2.5 萬例

**疫苗接種建議**：
- **嬰幼兒**：出生滿 2、4、6、18 個月及滿 5 歲至入學前各接種一劑
- **孕婦（重要）**：建議每次懷孕自費接種 1 劑 Tdap 疫苗，以懷孕第 27-36 週接種效益最大化
- **照顧者**：建議自費接種一劑 Tdap 疫苗

來源：tw_cdc_alerts/domestic_outbreak/2026-02-12-pertussis-first-case.md

##### 歷史參考 [語意搜尋]

| 日期 | 標題 | 來源 | 相關分數 |
|------|------|------|----------|
| 2026-02-04 | Pertussis: laboratory confirmed cases reported in England 2025 | UKHSA | 0.56 |
| 2025-12-19 | Latest figures show strong uptake of whooping cough vaccine in pregnancy | UKHSA | 0.48 |
| 2020-01-23 | Use of Tdap Vaccines: Updated Recommendations - United States, 2019 | US CDC MMWR | 0.46 |

#### 歐洲麻疹社區傳播 — EU/EEA

ECDC 於 2 月 9 日發布 2025 年麻疹監測分析，揭示歐盟/歐洲經濟區的麻疹疫情現況：

**病例統計**：
- 2025 年共報告 7,655 例麻疹病例（30 個國家）
- 較 2024 年的 35,000+ 例顯著下降
- 仍為 2023 年數據的兩倍
- 8 例死亡：法國 4 例、羅馬尼亞 3 例、荷蘭 1 例

**關鍵發現**：
- **傳播模式**：大多數病例透過國內社區傳播感染，而非境外輸入驅動
- **疫苗接種缺口**：80% 病例未接種疫苗
- **高風險族群**：五歲以下兒童佔 40%（3,072 例）

**建議措施**：

<blockquote class="expert-quote">
ECDC 指出：大多數病例透過國內社區傳播感染，而非境外輸入驅動。80% 病例未接種疫苗，五歲以下兒童佔 40%。建議各國達成 95% 兩劑麻疹疫苗覆蓋率。
<cite>— ECDC 麻疹監測分析, 2025</cite>
</blockquote>

<ol class="actionable-steps">
<li>達成 95% 兩劑麻疹疫苗覆蓋率</li>
<li>MMR 疫苗需兩週產生有效保護力</li>
<li>疫苗接種史不確定者應諮詢醫療人員</li>
</ol>

來源：ecdc_cdtr/situational_update/2026-02-09-community-spread-drives-ongoing-measles-tran.md

##### 歷史參考 [語意搜尋]

| 日期 | 標題 | 來源 | 相關分數 |
|------|------|------|----------|
| 2011-10-07 | Measles outbreaks: Regions of the Americas, Europe and Africa | WHO | 0.69 |
| 2019-05-02 | Progress Toward Measles Elimination - European Region, 2009-2018 | US CDC MMWR | 0.65 |
| 2024-04-11 | Measles - United States, January 2020-March 2024 | US CDC MMWR | 0.64 |

#### 英國 MMRV 疫苗計畫

英國自 2026 年 1 月 1 日起實施 MMRV（麻疹、腮腺炎、德國麻疹、水痘）疫苗計畫，將水痘保護納入常規兒童免疫接種：

**接種時程**：
- 12 個月大：第一劑
- 18 個月大：第二劑

**本週指引更新**（2/10）：
- 布洛芬使用指引
- 孕婦密切接觸者的免疫接種管理
- 補種世代資格澄清
- IT 系統相容性問題
- 現行感染兒童的疫苗接種規範

來源：uk_ukhsa_updates/guidance/2026-02-10-mmrv-programme-healthcare-practitioners.md

##### 歷史參考 [語意搜尋]

| 日期 | 標題 | 來源 | 相關分數 |
|------|------|------|----------|
| 2026-02-10 | MMRV programme: information for healthcare practitioners | UKHSA | 0.67 |
| 2026-01-22 | What is the MMRV vaccine and is my child eligible? | UKHSA | 0.61 |
| 2026-01-22 | Changes to the childhood vaccination schedule from January 2026 | UKHSA | 0.55 |

### 持續監測

#### 流感（Influenza）

- **全球活動度**：高
- **主要流行株**：A(H3N2) K 分支變異株
- **台灣疫情（更新至第 5 週 2/1-2/7）**：
  - 類流感門急診就診人次：110,870 人次（較前週下降 6.9%）
  - 上週（2/3-2/9）新增 32 例重症（H1N1 1 例、H3N2 27 例、B 型 4 例）、3 例死亡（均為 H3N2）
  - **本流感季累計 529 例重症、97 例死亡**
  - 重症病例：60% 為 65 歲以上長者，82% 具慢性病史，84% 未接種本季疫苗
  - 公費流感疫苗截至 2 月 9 日已接種約 673.5 萬劑，**剩餘約 9.3 萬劑**
- **鄰近國家**：
  - 韓國、日本流感呈上升趨勢
  - 韓國以 B 型為主，日本 H3N2 及 B 型共同流行
  - 全球主要流行型別：H3N2
- 來源：tw_cdc_alerts/policy/2026-02-10-cold-wave-lunar-new-year-flu-covid-policy.md

##### 歷史參考 [語意搜尋]

| 日期 | 標題 | 來源 | 相關分數 |
|------|------|------|----------|
| 2025-09-11 | Influenza-Associated Hospitalizations During a High Severity Season | US CDC MMWR | 0.64 |
| 2021-12-09 | Influenza A(H3N2) Outbreak on a University Campus - Michigan | US CDC MMWR | 0.63 |
| 2023-10-12 | High Influenza Incidence Among Children and Adolescents - 2022-23 Season | US CDC MMWR | 0.64 |

#### 禽流感 A(H5N1)

- **全球病例數**：2025 年共 30 例人類感染，12 例死亡
- **主要通報國家**：柬埔寨（18 例）、中國、美國、英國、越南、印度
- **2026 年至今**：截至 2 月 2 日，全球尚未確認人類 H5N1 病例
- **台灣禽場疫情**：2026 年截至 1 月 30 日已發生 7 起
- **旅遊警示**：
  - **第二級警示（Alert）**：中國（15 省市）、柬埔寨
  - **第一級注意（Watch）**：美國、英國、越南、印度等
- 來源：tw_cdc_alerts

##### 歷史參考 [語意搜尋]

| 日期 | 標題 | 來源 | 相關分數 |
|------|------|------|----------|
| 2025-03-13 | Human Cases of Highly Pathogenic Avian Influenza A(H5N1) - California | US CDC MMWR | 0.76 |
| 2024-05-30 | Outbreak of H5N1 in U.S. Dairy Cattle - Two Human Cases | US CDC MMWR | 0.67 |
| 2025-09-04 | H5N1 Infection in a Child with No Known Exposure - San Francisco | US CDC MMWR | 0.65 |

#### 立百病毒病（Nipah Virus Disease）

- **疫情地點**：印度西孟加拉邦（持續）、孟加拉（持續）
- **病例數**：印度西孟加拉邦截至 1/25 累計 5 例確診，其中 2 例重症
- **致死率**：WHO 資料顯示約 40%-75%
- **風險評估**：ECDC 評估對歐洲人風險極低；WHO 評估全球風險仍屬低度
- **政策動態**：台灣預計 3 月中正式將「立百病毒感染症」列為第五類法定傳染病
- 來源：ecdc_cdtr, tw_cdc_alerts

##### 歷史參考 [語意搜尋]

| 日期 | 標題 | 來源 | 相關分數 |
|------|------|------|----------|
| 2026-01-29 | Nipah virus disease cases reported in West Bengal, India | ECDC | 0.71 |
| 2026-01-29 | Nipah virus: what is it, where is it found and how does it spread? | UKHSA | 0.69 |

#### 屈公病（Chikungunya）

- **Level 2 警示國家**：
  - 蘇利南（2/5 新增）
  - 塞席爾（2/5 新增）
  - 古巴（9/26）
  - 斯里蘭卡（12/5）
- **傳播趨勢**：持續擴散至新地區
- **防護措施**：防蚊措施，包括使用防蚊液、穿長袖長褲、住有紗窗空調處
- 來源：us_travel_health_notices

##### 歷史參考 [語意搜尋]

| 日期 | 標題 | 來源 | 相關分數 |
|------|------|------|----------|
| 2025-12-22 | Chikungunya Virus Disease Worldwide Overview | ECDC | 0.62 |
| 2023-06-08 | Chikungunya Outbreak - Paraguay, 2022-2023 | US CDC MMWR | 0.57 |
| 2025-12-05 | Level 2 - Chikungunya in Sri Lanka | US CDC Travel | 0.56 |

#### 猴痘（Mpox）

**英國流行病學概況更新（2/12）**：
- UKHSA 更新 Mpox clade Ib 及 clade IIb 疫情監測報告
- 自 2022 年 6 月起追蹤 clade IIb 病例
- 2025 年 1 月後納入 clade Ib 監測數據
- 來源：uk_ukhsa_updates/surveillance/2026-02-12-mpox-clade-ib-and-iib-outbreak-epidemiological-overview.md

**台灣疫情（2/10 更新）**：
- 2026 年 1 月新增 5 例本土確定病例（皆為本國籍青壯年男性）
- 地區分布：北部 4 例、中部 1 例
- 累計（自 2022/6/23 起至 2026/2/9）：518 例確診（483 例本土、35 例境外移入）
- 全國 312 家醫療院所提供公費 M 痘疫苗接種服務

**全球疫情**：
- 2025 年全球累計：98 國報告 52,000 餘例，215 例死亡
- 主要疫區：非洲（剛果民主共和國、幾內亞等，以 Ib 型為主）
- 歐洲社區傳播：法國、義大利、西班牙出現 Ib 型社區傳播

**旅遊警示**：
- US CDC 對迦納及賴比瑞亞維持 Level 2 旅遊健康通知（Clade II 猴痘疫情）
- 來源：us_travel_health_notices, tw_cdc_alerts

##### 歷史參考 [語意搜尋]

| 日期 | 標題 | 來源 | 相關分數 |
|------|------|------|----------|
| 2025-01-14 | Epidemiological update: Mpox due to monkeypox virus clade I | ECDC | 0.72 |
| 2024-08-16 | Risk assessment for the EU/EEA of the mpox epidemic caused by monkeypox virus clade I in affected African countries | ECDC | 0.62 |
| 2025-02-13 | First Clade Ib Monkeypox Virus Infection Reported in the Americas - California | US CDC MMWR | 0.62 |

#### COVID-19

- **台灣疫情**：
  - 第 5 週門急診就診人次：1,035 人次（較前週上升 8.8%）
  - 上週新增 3 例本土重症病例
  - 114 年 10 月起累計 63 例重症、6 例死亡
  - 重症高風險族群：65 歲以上長者占 70%，具慢性病史占 81%，92% 未接種本季疫苗
- **全球趨勢**：
  - 近期病毒陽性率略升（東南亞及東地中海區域明顯增加）
  - 中國、日本、韓國、印尼疫情上升
- **流行變異株**：全球以 XFG 占比最高，鄰近國家以 NB.1.8.1 為主
- 來源：tw_cdc_alerts/policy/2026-02-10-cold-wave-lunar-new-year-flu-covid-policy.md

#### 諾羅病毒（Norovirus）— 春節腹瀉風險

- **台灣疫情**：近期腹瀉群聚案件以諾羅病毒為主，春節期間急性腹瀉就診人數較平日增加
- **傳染特性**：傳染力非常強，可藉由排泄物或嘔吐物人傳人
- **高風險群**：嬰幼兒、年長者、免疫低下者（症狀較嚴重）
- **防護措施**：
  - 勤洗手（使用肥皂）
  - 食物充分加熱，避免生、熟食交叉汙染
  - 確保食材新鮮
  - 有症狀者應在家休息至症狀解除至少 48 小時
- 來源：tw_cdc_alerts/policy/2026-02-13-spring-festival-epidemic-prevention.md

#### 登革熱（Dengue）

- **全球風險**：全年持續，每 2-5 年爆發
- **高風險地區**：東南亞、美洲多國
- **旅遊建議**：Level 1 Watch，建議防蚊措施
- 來源：us_travel_health_notices, tw_cdc_alerts

### 風險評估更新

#### RSV 疫苗接種覆蓋率 — 英國

UKHSA 於 2 月 12 日發布 RSV（呼吸道合胞病毒）疫苗接種覆蓋率報告：

**孕婦疫苗接種計畫**：
- 追蹤孕婦 RSV 疫苗接種覆蓋率
- 評估透過母體免疫保護新生兒的計畫成效

**老年人疫苗接種計畫**：
- 60 歲以上成人 RSV 疫苗接種覆蓋率數據
- 搭配上週公布的 RSV 疫苗擴展計畫（已獲核准）

來源：
- uk_ukhsa_updates/surveillance/2026-02-12-rsv-maternal-vaccination-coverage-in-england.md
- uk_ukhsa_updates/surveillance/2026-02-12-rsv-older-adults-vaccination-coverage-in-england-statistics.md

##### 歷史參考 [語意搜尋]

| 日期 | 標題 | 來源 | 相關分數 |
|------|------|------|----------|
| 2026-02-12 | RSV older adults vaccination coverage in England | UKHSA | 0.72 |
| 2024-05-30 | Early Safety Findings Among Persons Aged 60+ Who Received RSV Vaccine | US CDC MMWR | 0.65 |
| 2024-09-26 | Maternal RSV Vaccination and Receipt of Nirsevimab by Infants | US CDC MMWR | 0.64 |

#### 肺炎鏈球菌疾病監測 — 英國

UKHSA 本週更新侵襲性肺炎鏈球菌疾病（IPD）監測報告：

**監測範圍**：
- PCV13 疫苗未涵蓋血清型病例
- 追蹤期間：2017 年 7 月至 2025 年 9 月（第 39 週）

**年齡分層**：
- 2 歲以下兒童
- 2-4 歲兒童
- 5-64 歲成人
- 65 歲以上長者

**公衛意義**：
- 識別疫苗覆蓋缺口
- 監測疫苗接種後族群的疾病模式
- 偵測新興肺炎鏈球菌威脅
- 為未來疫苗政策提供依據

來源：uk_ukhsa_updates/surveillance/2026-02-10-pneumococcal-disease-non-pcv13-serotypes.md

---

## 區域動態

### 歐洲 (ECDC)

**第 7 週傳染病威脅報告（2026/2/7-2/13）**

| 威脅項目 | 狀態 |
|----------|------|
| 呼吸道病毒流行病學 | EU/EEA 持續監測 |
| 禽流感 (Avian influenza) | 持續監測 |
| 屈公病 (Chikungunya) | 全球監測 |
| 登革熱 (Dengue) | 全球監測 |
| 立百病毒病 (Nipah) | 印度西孟加拉邦持續監測 |
| 麻疹 (Measles) | 社區傳播專題分析 |
| 大型集會監測 | 冬季奧運進行中 |

**本週重要發布**：
- **CDTR 第 7 週報告**（2/13）：涵蓋呼吸道病毒、禽流感、屈公病、登革熱、尼帕病毒、麻疹等威脅
- **麻疹社區傳播分析**（2/9）：2025 年全年監測報告，7,655 例病例，80% 未接種疫苗

### 英國 (UKHSA)

**本週重要更新**

| 類型 | 事件 | 日期 |
|------|------|------|
| **監測報告** | **Mpox clade Ib/IIb 流行病學概況** | **2/12** |
| **監測報告** | **RSV 孕婦疫苗接種覆蓋率** | **2/12** |
| **監測報告** | **RSV 老年人疫苗接種覆蓋率** | **2/12** |
| 指引更新 | Brucella 實驗室與臨床服務 | 2/12 |
| 指引更新 | MMRV 疫苗計畫醫療專業人員指引 | 2/10 |
| 監測報告 | 肺炎鏈球菌非 PCV13 血清型監測 | 2/10 |
| 政策報告 | UKHSA 保障年度報告 2024-2025 | 2/9 |
| 指引更新 | 公共衛生資源索引 | 2/11 |

**疫苗計畫進展**：
- MMRV 疫苗計畫持續實施（2026 年 1 月起）
- RSV 疫苗擴展方案已獲核准，並發布首批接種覆蓋率數據

### 亞太地區

**台灣 (tw_cdc_alerts)**

| 事件 | 日期 | 重點 |
|------|------|------|
| **今年首例麻疹境外移入** | 2/12 | 北部 8 個月大男嬰，越南感染，400 名接觸者監測中 |
| **今年首例百日咳確診** | 2/12 | 北部 5 個月大男嬰，全球疫情嚴峻（日本去年逾 8 萬例） |
| HIV/M痘防疫宣導 | 2/10 | 新春情人節篩檢優惠、M痘疫苗接種提醒 |
| 狂犬病預防 | 2/10 | 春節戶外活動避免接觸野生動物，野生動物確診持續 |
| 春節出國旅遊健康提醒 | 2/5 | 流感、麻疹、登革熱、屈公病、M痘風險警示 |
| 禽流感防護提醒 | 2/3 | 「5要6不」原則，全球 H5N1 監測 |
| 流感疫情更新 | 2/3 | 新增 25 例重症、1 例死亡，公費疫苗剩約 11 萬劑 |

**政策更新**：
- 1/20-2/28 擴大公費流感抗病毒藥劑使用條件（不需快篩即可開立）
- 1/1-2/28 擴大 COVID-19 疫苗接種對象
- 公費流感疫苗剩餘約 9.3 萬劑
- **春節傳染病特別門診（新增）**：
  - 初一至初三（2/17-2/19）共 163 家醫院配合開設
  - 總計約 1,372 診次（初一 447、初二 453、初三 472）
  - 服務範圍：呼吸道及腸胃道傳染病
  - 查詢：https://gov.tw/gvq

**旅遊疫情建議**：
- **麻疹第二級警示（Alert）**：越南、印尼、印度、安哥拉、墨西哥、葉門、巴基斯坦（共 7 國）
- **麻疹第一級注意（Watch）**：31 國

**印度**
- 西孟加拉邦立百病毒疫情持續監測中

**日本**
- 百日咳疫情嚴峻：2025 年全年逾 8 萬例（歷年最高），多地發現抗藥性菌株

### 美洲

**旅遊健康通知**：
- **Level 2（維持）**：蘇利南 — 屈公病（2/5）
- **Level 2（維持）**：塞席爾 — 屈公病（2/5）
- **Level 2（維持）**：迦納、賴比瑞亞 — Clade II 猴痘（1/26）
- **Level 1（維持）**：全球登革熱風險（1/22）

**麻疹疫情**：美國、加拿大、墨西哥近期持續有病例發生

---

## 旅遊健康建議

### 春節連假特別提醒

疾管署針對 2026 年農曆春節期間出國旅遊發布全面性防疫提醒：

#### 呼吸道傳染病

| 疾病 | 高風險地區 | 防護建議 |
|------|------------|----------|
| 流感 | 日本、韓國、香港、中國 | 行前接種疫苗、戴口罩、手部衛生 |
| 麻疹 | 美國、加拿大、墨西哥、印度、印尼、越南、歐洲 | 確認 MMR 疫苗接種狀態 |
| 新型 A 型流感 | 中國、柬埔寨、美國 | 避免接觸禽鳥及野生動物 |
| COVID-19 | 中國、日本、東南亞 | 評估接種追加劑 |

#### 蟲媒傳染病

| 疾病 | 高風險地區 | 防護建議 |
|------|------------|----------|
| 登革熱 | 東南亞、美洲多國 | 穿淺色長袖、使用防蚊藥劑 |
| 屈公病 | 蘇利南、塞席爾、古巴、斯里蘭卡、東南亞 | 選擇有紗窗紗門之住宿環境 |
| 瘧疾 | 非洲、印度、印尼 | 提前一個月至旅醫門診評估預防藥物 |

#### 食媒傳染病

| 疾病 | 注意事項 |
|------|----------|
| 霍亂、A 型肝炎 | 飲用瓶裝或煮沸水、避免生食 |
| 諾羅病毒 | 熟食熱食、勤洗手 |

### Level 2 - 加強防護措施

| 目的地 | 疾病 | 建議 |
|--------|------|------|
| 蘇利南 | 屈公病 (Chikungunya) | 防蚊措施：使用防蚊液、穿長袖長褲、住有紗窗空調處 |
| 塞席爾 | 屈公病 (Chikungunya) | 防蚊措施：使用防蚊液、穿長袖長褲、住有紗窗空調處 |
| 迦納、賴比瑞亞 | 猴痘 (Clade II) | 避免接觸病患、野生動物、野味；注意安全性行為 |
| 中國（15省市）、柬埔寨 | 禽流感 H5N1 | 避免接觸禽鳥及野生動物，禽肉蛋類應完全煮熟 |

### Level 1 - 一般防護措施

| 目的地 | 疾病 | 建議 |
|--------|------|------|
| 全球多國 | 登革熱 | 使用防蚊液、穿長袖長褲、使用紗窗/蚊帳 |
| 全球多國 | 麻疹 | 確認 MMR 疫苗接種狀態 |
| 美國、英國、越南、印度 | 禽流感 H5N1 | 避免接觸禽鳥 |

### 其他旅遊注意事項

**立百病毒流行區（印度西孟加拉邦、喀拉拉州；孟加拉）**
- 避免接觸蝙蝠或豬隻
- 避免飲用生椰棗樹汁及食用可能被污染的水果
- 保持良好手部衛生

**麻疹高風險地區**
- 確認 MMR 疫苗接種狀況
- 疫苗需兩週產生有效保護力
- 疫苗接種史不確定者應諮詢醫療人員

---

## 公共衛生指引

### 疫苗接種

**MMRV 疫苗（英國）**
- 自 2026 年 1 月 1 日起實施
- 接種時程：12 個月及 18 個月
- 涵蓋：麻疹、腮腺炎、德國麻疹、水痘

**麻疹預防**

<p class="key-answer" data-question="如何預防麻疹">預防麻疹最有效的方式是接種 MMR 疫苗，需達成 95% 兩劑覆蓋率才能阻斷社區傳播，疫苗需兩週產生有效保護力。</p>

<ol class="actionable-steps">
<li>達成 95% 兩劑麻疹疫苗覆蓋率</li>
<li>MMR 疫苗需兩週產生有效保護力</li>
<li>疫苗接種史不確定者應諮詢醫療人員</li>
<li>6 個月以上未滿 1 歲嬰兒前往流行區，可出發前 2-4 週自費接種 1 劑 MMR</li>
</ol>

**百日咳預防**

<p class="key-answer" data-question="如何預防百日咳">預防百日咳的關鍵是疫苗接種：嬰幼兒依時程接種五劑，孕婦建議於懷孕第 27-36 週接種 Tdap 疫苗，以將母體抗體傳遞給新生兒。</p>

<ol class="actionable-steps">
<li><strong>嬰幼兒常規接種</strong>：出生滿 2、4、6、18 個月及滿 5 歲至入學前各接種一劑</li>
<li><strong>孕婦接種（重要）</strong>：建議每次懷孕自費接種 1 劑 Tdap 疫苗，以懷孕第 27-36 週接種效益最大化，可將母體抗體傳遞給新生兒</li>
<li><strong>照顧者接種</strong>：建議嬰兒照顧者自費接種一劑 Tdap 疫苗</li>
<li><strong>日常防護</strong>：家長及較大兒童注意咳嗽禮節，外出返家接觸嬰幼兒前先更衣洗手</li>
</ol>

### 禽流感預防「5要6不」原則（台灣）

<p class="key-answer" data-question="如何預防禽流感">台灣疾管署建議民眾落實「5要6不」原則預防禽流感：禽肉蛋類完全煮熟、落實手部衛生、避免接觸禽鳥及野生動物。</p>

**5要：**
<ol class="actionable-steps">
<li>禽肉及蛋類應完全煮熟</li>
<li>落實手部衛生</li>
<li>有呼吸道症狀時應戴口罩</li>
<li>符合接種資格者應接種流感疫苗</li>
<li>維持均衡營養及運動習慣</li>
</ol>

**6不：**
<ol class="actionable-steps">
<li>不生食禽鳥相關製品</li>
<li>不購買來路不明禽鳥</li>
<li>不接觸禽鳥</li>
<li>不棄養禽鳥</li>
<li>不混養不同種類禽畜</li>
<li>不進入人潮擁擠、空氣不流通場所</li>
</ol>

### 流感及 COVID-19 預防

- 符合資格者應儘速接種公費疫苗
- 落實勤洗手、咳嗽禮節
- 出現呼吸困難、胸痛、意識改變等危險徵兆應儘速就醫

---

## 資料來源

本報告基於以下來源：
- ECDC 傳染病威脅報告 (ecdc_cdtr)
- WHO 疾病疫情新聞 (who_disease_outbreak_news)
- US CDC 健康警報網絡 (us_cdc_han)
- US CDC MMWR 週報 (us_cdc_mmwr)
- US CDC 旅遊健康通知 (us_travel_health_notices)
- 台灣疾管署公告 (tw_cdc_alerts)
- UK Health Security Agency (uk_ukhsa_updates)

### 本週引用資料

**ECDC (ecdc_cdtr)**
- `surveillance/2026-02-13-communicable-disease-threats-report-7-13-february-2026-week-7.md` **[本週新增]**
- `situational_update/2026-02-09-community-spread-drives-ongoing-measles-tran.md`
- `surveillance/2026-02-06-communicable-disease-threats-report-week-6.md`
- `guidance/2026-02-05-winter-olympics-and-paralympics-2026-health-protection.md`

**台灣疾管署 (tw_cdc_alerts)**
- `policy/2026-02-13-spring-festival-epidemic-prevention.md` **[本週新增]**
- `imported_case/2026-02-12-measles-imported-case-vietnam.md` **[本週新增]**
- `domestic_outbreak/2026-02-12-pertussis-first-case.md` **[本週新增]**
- `policy/2026-02-10-cold-wave-lunar-new-year-flu-covid-policy.md` **[本週新增]**
- `medical_advisory/2026-02-10-lunar-new-year-hiv-mpox-prevention.md` **[本週新增]**
- `disease_info/2026-02-10-rabies-prevention-outdoor-activities.md` **[本週新增]**
- `policy/2026-02-05-spring-festival-travel-health-advisory.md`
- `policy/2026-02-03-cny-travel-disease-prevention.md`
- `policy/2026-02-03-avian-influenza-winter-prevention.md`
- `disease_info/2026-02-08-measles.md`
- `disease_info/2026-02-08-pertussis.md`

**UK UKHSA (uk_ukhsa_updates)**
- `guidance/2026-02-13-tuberculosis-tb-factsheet.md` **[本週新增]**
- `surveillance/2026-02-12-mpox-clade-ib-and-iib-outbreak-epidemiological-overview.md` **[本週新增]**
- `surveillance/2026-02-12-rsv-maternal-vaccination-coverage-in-england.md` **[本週新增]**
- `surveillance/2026-02-12-rsv-older-adults-vaccination-coverage-in-england-statistics.md` **[本週新增]**
- `guidance/2026-02-12-brucella-laboratory-and-clinical-services.md` **[本週新增]**
- `guidance/2026-02-10-mmrv-programme-healthcare-practitioners.md`
- `surveillance/2026-02-10-pneumococcal-disease-non-pcv13-serotypes.md`
- `surveillance/2026-02-05-outbreaks-under-monitoring-in-2026.md`
- `policy/2026-02-09-ukhsa-safeguarding-annual-report-2024-2025.md`

**US CDC (us_travel_health_notices)**
- `level_2_practice/2026-02-05-chikungunya-suriname.md`
- `level_2_practice/2026-02-05-chikungunya-seychelles.md`
- `level_2_practice/2026-01-26-monkeypox-ghana-liberia.md`
- `level_1_watch/2026-01-22-dengue-global.md`

### 語意搜尋查詢紀錄

本報告使用 Qdrant 語意搜尋提供歷史脈絡，執行以下查詢：

| 查詢 | 找到筆數 | 最高相關分數 |
|------|----------|--------------|
| measles outbreak 2026 | 10 | 0.69 |
| mpox clade Ib outbreak 2025 2026 | 10 | 0.79 |
| avian influenza H5N1 bird flu | 10 | 0.68 |
| nipah virus India outbreak | 10 | 0.67 |
| chikungunya outbreak Suriname 2025 2026 | 10 | 0.69 |
| respiratory virus surveillance influenza COVID RSV | 10 | 0.68 |
| pertussis whooping cough outbreak | 5 | 0.56 |
| RSV respiratory syncytial virus vaccine | 5 | 0.72 |
| H5N1 avian influenza human cases 2025 | 10 | 0.76 |
| influenza A H3N2 outbreak season | 8 | 0.64 |

---
*報告產出時間: 2026-02-15T12:00:00+08:00*
*上次更新: 2026-02-16T05:00:00+08:00*
*下次更新: 2026-02-23 (2026-W08 週報)*
