...
> **快速指令**：在專案根目錄的 CLAUDE.md 中使用「執行網站改版」或「網站健檢」即可觸發。
...
在 `lib/` 目錄提供可直接執行的腳本（使用本地 Lighthouse，不受 API 配額限制）：

### site-audit.sh - 網站健檢

```bash
# 完整健檢
lib/site-audit.sh https://example.com

# 只執行 Lighthouse
lib/site-audit.sh https://example.com --lighthouse

# 只檢測安全性
lib/site-audit.sh https://example.com --security

# 只檢測 SEO
lib/site-audit.sh https://example.com --seo

# 輸出到檔案
lib/site-audit.sh https://example.com --output report.json
```
...
```bash
lib/competitive-audit.sh https://our-site.com https://competitor1.com https://competitor2.com
```
