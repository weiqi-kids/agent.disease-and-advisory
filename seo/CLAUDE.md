...
> **本區塊為 EpiAlert 疫情快訊網站的固定設定，Writer 必須使用這些值。**
...
| `SITE_DESCRIPTION` | `全球傳染病情報自動收集與分析` |
| `SITE_LANGUAGE` | `zh-TW` |

### Organization Schema（固定）
...
由於疫情資訊屬於 YMYL（Your Money or Your Life）內容，所有頁面必須包含以下免責聲明：

```html
<div class="ymyl-disclaimer">
  本站內容由 AI 自動彙整自 WHO、CDC 等官方來源，僅供參考，不構成醫療建議。
  如有健康疑慮，請諮詢專業醫療人員。
</div>
```

**YMYL 專屬欄位**：
- `lastReviewed`：每次產出報告時更新為當前日期
- `reviewedBy`：固定為 `EpiAlert AI 編輯`
- `medicalDisclaimer`：固定使用上方免責聲明

### EpiAlert 適用的 Schema 組合

| 頁面類型 | 必填 Schema | 條件式 Schema |
|----------|------------|--------------|
| **首頁** | WebPage, Organization | - |
| **週報** | WebPage, Article, Person, Organization, BreadcrumbList | ItemList（本週重點列表） |
| **萃取結果** | WebPage, Article, Person, Organization, BreadcrumbList | - |
| **索引頁** | WebPage, Organization, BreadcrumbList | ItemList |

### 不適用的 Schema

以下 Schema 在 EpiAlert 中**不適用**，Writer 可跳過：

- `FAQPage` — 疫情報告不適合 FAQ 形式
- `Recipe` — 非料理內容
- `Product` — 非商品頁面
- `LocalBusiness` — 非在地商家
- `Course` — 非課程內容
- `Event` — 疫情事件不適合 Event Schema
- `Review` — 非評測內容
...
| 原規則 | EpiAlert 調整 |
|--------|--------------|
| `.key-answer` + data-question | 適用於週報各段落 |
| `.key-takeaway` | 適用於「本週重點」區塊 |
| `.expert-quote` | 引用官方來源時使用 |
